...
## Acceptance Criteria
- [x] A "Load character…" toolbar action is reachable from every section of the workspace
- [x] Opening it shows a popup with an empty 4-slot file widget; the current character and workspace remain fully visible and usable underneath
- [x] A failed/partial load in the popup leaves the original character and every section's state completely unaffected
- [x] A successful load replaces the character, resets every section's own selection state to default, but keeps the currently active sidebar section selected
