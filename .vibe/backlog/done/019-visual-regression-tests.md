...
- [x] Baseline screenshots exist for: a decoded sprite in the sprite browser, the animation player on a specific frame with the Clsn overlay on and off, and the palette picker's live-recolored preview after applying an override `.act`
- [x] `npm run test:visual` runs these in CI as its own job, separate from `npm test`, and fails the build on a diff
- [x] A real, deliberate rendering regression (verified by temporarily breaking one of the covered paths, then reverting) is caught by this suite
