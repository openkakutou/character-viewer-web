...
## Acceptance Criteria
- [x] A Preferences icon in the toolbar opens a popup with a "Beginner mode" toggle, off by default
- [x] Enabling it shows an info icon next to each MUGEN/Ikemen technical term across the Characteristics, Palette, Sprites, and Animation sections; hovering/focusing it reveals a plain-language explanation
- [x] The setting persists for the rest of the session (not just the currently open section) and survives a character switch (item 021)
- [x] Disabling it hides every info icon again, with no other visual change
