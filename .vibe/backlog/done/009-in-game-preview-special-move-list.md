...
## Acceptance Criteria
- [x] User sees a list of the loaded character's Statedefs
- [x] Clicking a Statedef entry plays that state's associated animation in the preview (reusing item 007's player)
- [x] A Statedef with no clearly associated animation (e.g. no `ChangeAnim`-equivalent data available) shows a clear "no animation" state instead of failing silently
- [x] Triggering a new state while another is playing cleanly replaces it, same as item 008's animation triggers
