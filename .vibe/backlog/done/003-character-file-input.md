...
## Acceptance Criteria
- [x] User can select the four files via a file picker, or drag-and-drop them onto a drop zone
- [x] Selected files are read as byte buffers and passed to the WASM bridge's load call
- [x] A missing required file (e.g. only 3 of 4 provided) shows a clear error state naming which file is missing, instead of calling the bridge with incomplete data
- [x] An unreadable/corrupt file selection shows a clear error state instead of crashing the page
