...
// recursive `FileSystemDirectoryReader` walk — see backlog item 015's own
...
// does not). Ported near-verbatim from `character-editor`'s own
// `src/input/folder-entries.ts` (itself ported from `stage-editor`/
// `lifebar-viewer-web`) — generic gathering logic, no domain specifics. See
