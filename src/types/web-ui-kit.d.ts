...
// 018): the installed package ships no `.d.ts` of its own -- its
...
// everything else `any` too -- ported verbatim from the sibling
// `stage-viewer-web`/`lifebar-viewer-web`/`character-editor` repos' own
// identical file for the same gap. The real fix (the package shipping its
// own declarations) belongs in `web-ui-kit` itself.
//
...
// confirmed by the sibling repos' own equivalent file via bisection, cause
// not otherwise diagnosed -- so this app avoids that combination entirely
// rather than depending on unconfirmed compiler behavior.
